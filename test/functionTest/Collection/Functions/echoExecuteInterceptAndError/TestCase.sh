...
function testStep {
	case "$TTRO_variantCase" in
		...
		emptyCommand)
			echoExecuteInterceptAndError '';;
		wrongCommand)
			echoExecuteInterceptAndError "bxbxbx"
			if [[ $TTTT_result -ne 127 ]]; then
				setFailure "returncode is $TTTT_result and not 127"
			fi;;
		succ)
			echoExecuteInterceptAndError 'returnSucc'
			if [[ $TTTT_result -ne 0 ]]; then
				setFailure "returncode is $TTTT_result and not 0"
			fi;;
		fails)
			echoExecuteInterceptAndError 'returnFail'
			if [[ $TTTT_result -ne 1 ]]; then
				setFailure "returncode is $TTTT_result and not 1"
			fi;;
		simpleCommands1Succ)
			var="Command2"
			echoExecuteInterceptAndError 'expect3Params' 'Command1' "$var" '$var'
			if [[ $TTTT_result -ne 0 ]]; then
				setFailure "returncode is $TTTT_result and not 0"
			fi;;
		simpleCommands2Succ)
			var=''
			echoExecuteInterceptAndError 'expect3Params' 'Command1' "$var" ''
			if [[ $TTTT_result -ne 0 ]]; then
				setFailure "returncode is $TTTT_result and not 0"
			fi;;
		simpleCommands27)
			var=''
			echoExecuteInterceptAndError 'expect3Params' 'special' "$var" ''
