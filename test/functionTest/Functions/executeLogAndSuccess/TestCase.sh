...
function testStep {
	case "$TTRO_variantCase" in
		...
		simpleCommands2Succ)
			var=''
			executeLogAndSuccess 'expect3Params' 'Command1' "$var" ''
			if [[ $TTTT_result -ne 0 ]]; then
				setFailure "returncode is $TTTT_result and not 0"
			fi
			if ! linewisePatternMatch "$TT_evaluationFile" '' 'expect3Params*'; then setFailure; fi;;
		simpleCommands27)
			var=''
			executeLogAndSuccess 'expect3Params' 'special' "$var" ''
